...
def it_deepening_nm(turn, game, time_limit):
    start = time.time()
    ...
    def nega_max(turn, depth, alpha, beta, game):
        ...
        # order moves
        for score, state, piece in states:
    ...
    state_scores = []
    depth = 1
    # last_best_state = turn
    last_best = None
    while True:
        if time.time() - start >= time_limit:
            break

        score, next_state, piece_to_move = nega_max(turn, depth, -1000, 1000, game)

        last_best = score, next_state, piece_to_move
        # last_best_state = next_state
